...
export function activate(context: vscode.ExtensionContext): void {
	...
	statusBar.command = 'tokenPie.reveal';
	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand('tokenPie.reveal', () => reveal()),
...
function updateStatusBar(): void {
	...
	if (busy) {
		...
		statusBar.tooltip =
			...
			'the figures appear once this finishes.';
	...
	statusBar.command = degraded ? 'tokenPie.showLogs' : 'tokenPie.reveal';
	...
	statusBar.tooltip = degraded
		? `Token Pie read what it could, but ${lastErrors.length} problem` +
		  ...
		  'the usage view is in the activity bar.'
		: buildTooltip(p);
	statusBar.show();
}
...
function buildTooltip(p: Projection): vscode.MarkdownString {
	...
	md.appendMarkdown(`\n_${p.quotaId ?? 'quota'} — click to open the usage view_`);
	return md;
}
...
/**
 * What the status bar click does now.
 *
 * It opened a full editor tab, which made sense when that was the only surface
 * and is redundant beside a view that is one click away and already open half
 * the time. Revealing the view costs no editor space and lands you on the same
 * figures. The panel is still there for the wide tables, on the palette and on
 * the view's title bar.
 *
 * `<viewId>.focus` is generated by VS Code for every contributed view. If a
 * host ever stops providing it the panel is the honest fallback -- better a
 * tab than a click that does nothing.
 */
async function reveal(): Promise<void> {
	try {
		await vscode.commands.executeCommand('tokenPie.sidebar.focus');
	} catch {
		if (extensionContext) {
			showReport(extensionContext);
		}
	}
}
